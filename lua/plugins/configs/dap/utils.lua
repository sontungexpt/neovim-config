...
M.continue_debugging = function()
	vim.schedule(function()
		...
		while true do
			...
			if status == 0 then
				logger.info(lang.msg.succeed or "job succeed")
				require("dap").continue()
				break
			elseif status == -1 then
				-- Job was stopped (e.g. user cancelled the debugging session)
				fn.jobstop(job_id)
				logger.error("job stopped suddenly", { title = "Dap continue error" })
				break
			else
				-- Job failed to start or encountered an error
				fn.jobstop(job_id)
				logger.error(lang.msg.failed, { title = "Dap continue error" })
				break
			end
			...
			if elapsed_time / 1e6 >= timeout then
				-- Job timed out
				fn.jobstop(job_id)
				logger.error(lang.msg.timed_out, { title = "Dap continue error" })
				break
			end
		end
	end)
